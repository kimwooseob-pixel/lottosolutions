...
function initGame() {
    ...
    if (stored) {
        try {
            ...
            const nums = Array.isArray(parsed) ? parsed : (parsed && parsed.numbers);
            if (Array.isArray(nums)) {
                gameState.winningNumbers[gameState.endNumber] = nums;
            }
        } catch (e) {
            console.error('winningNumbers localStorage 파싱 오류:', e);
        }
    }
